...
2. 任务启停统一切换端点：`PATCH /api/projects/{id}` + `{is_active: true/false}`（前端首选，来源：Spec 007）；`DELETE /api/projects/{id}` 保留给 Admin 使用，语义为 `is_active=false`
...
10. `GET /api/feeds/{id}/preview_html`：同上 API 路由入口；`FeedHtmlPreviewView` 豁免 `X-Frame-Options`（`@xframe_options_exempt`），允许同源 iframe 嵌入预览（来源：Spec 004 + Spec 007）
