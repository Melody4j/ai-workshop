...
8. 当前任务停用不清理历史报告记录
9. `DataSnapshot` 数据库字段只存绝对文件路径（`raw_html_path` / `raw_md_path` / `clean_md_path`），不存内容；内容为空时路径为空字符串
10. `DataSnapshot.raw_md_path` 指向 Firecrawl Markdown 原文，用于原始证据 diff
11. `DataSnapshot.clean_md_path` 指向 LLM 降噪后 MD（文件名 `llm_` 前缀标识），用于页面理解/报告上下文，不再作为 diff 真相源
12. 旧格式快照兼容：缺少 `raw_md_path`，或 `clean_md_path` 无 `llm_` 前缀时，视为 pre-stable-diff 格式，跳过 diff（Spec 004/010 兼容）
13. `DataSnapshot` append-only——禁止 UPDATE/DELETE（DB 触发器尚未实现，见 Evidence Gaps）
14. `IntelligenceFeed.push_status` 保持兼容 `NOT_PUSHED` / `PUSHED` / `PUSH_FAILED`，默认 `NOT_PUSHED`（来源：Spec 005）
15. `push_status` 与 `job_status` 正交——`job_status` 标识情报结果，`push_status` 标识推送结果，不互相覆盖（来源：Spec 005）
16. 仅 `job_status=CHANGED` 的记录触发推送（来源：Spec 005）
17. `IntelligenceFeed.raw_diff_text` 表示 Firecrawl Markdown 的原始 unified diff；若 diff 计算后进入 ERROR_CRAWL / CHANGED / NO_CHANGE，应尽量保留该证据
18. `IntelligenceFeed.diff_text` 表示 Firecrawl Markdown 经规则归一化后的稳定 diff；首次爬取/旧格式兼容路径仍允许写入 `llm_clean_md` 全量内容
19. `PromptVersion` 存储 prompt 全文版本（prompt_name / content / version / feed FK SET_NULL / optimization_reason），`version` 按 `prompt_name` 分组自增（来源：Spec 006）
20. `PromptVersion.feed` 使用 `SET_NULL`，feed 删除后版本记录保留（来源：Spec 006）
